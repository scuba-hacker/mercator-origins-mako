...
//  uart_set_mode(uart_number, UART_MODE_RS485_HALF_DUPLEX);
  ...
  magnetometer_min = (vector<float>){-51.15, -60.45, 0.00};
  magnetometer_max = (vector<float>){53.10, 37.05, 110.25}; 
...
void sendUplinkTelemetryMessageV3()
{
  ...
  if (millis() > latestFixTimeStamp + quietTimeMsBeforeUplink)
  {
    ...
    uint8_t number_uplink_metrics=59;
    uint16_t* nextMetric=telemetryMessage;
...
void sendUplinkTelemetryMessageV4()
{
  // if 1ms after fix received then send the uplink msg, only get here if no bytes received from Float Serial.
  const uint32_t quietTimeMsBeforeUplink = 5; 
  if (millis() > latestFixTimeStamp + quietTimeMsBeforeUplink)
  {
    ...
    uint8_t number_uplink_metrics=37;
    uint16_t* nextMetric=telemetryMessage;
    ...
    *(nextMetric++) = uplink_mako_bat_charge_current;    
    ...
    char* p = (char*) &uplink_mako_lsm_mag_x;
    *(nextMetric++)=*(p++) | (*(p++)<<8);*(nextMetric++)=*(p++) | (*(p++)<<8);

    p = (char*) &uplink_mako_lsm_mag_y;
    *(nextMetric++)=*(p++) | (*(p++)<<8);*(nextMetric++)=*(p++) | (*(p++)<<8);

    p = (char*) &uplink_mako_lsm_mag_z;
    *(nextMetric++)=*(p++) | (*(p++)<<8);*(nextMetric++)=*(p++) | (*(p++)<<8);

    p = (char*) &uplink_mako_lsm_acc_x;
    *(nextMetric++)=*(p++) | (*(p++)<<8);*(nextMetric++)=*(p++) | (*(p++)<<8);

    p = (char*) &uplink_mako_lsm_acc_y;
    *(nextMetric++)=*(p++) | (*(p++)<<8);*(nextMetric++)=*(p++) | (*(p++)<<8);

    p = (char*) &uplink_mako_lsm_acc_z;
    *(nextMetric++)=*(p++) | (*(p++)<<8);*(nextMetric++)=*(p++) | (*(p++)<<8);
...
bool getMagHeadingTiltCompensated(float& tiltCompensatedHeading)
{
  float tch = magHeading((vector<int>) {1, 0, 0});
